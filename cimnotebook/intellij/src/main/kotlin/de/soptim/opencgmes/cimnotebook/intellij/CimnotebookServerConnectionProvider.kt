...
/**
 * Launches cimvocabcheck-lsp.jar as a subprocess and connects to it via stdio.
 *
 * JAR resolution order (same as the VS Code extension):
 *  1. Explicit path in Settings > Tools > CIMNotebook > Server JAR.
 *  2. cimvocabcheck-lsp.jar bundled inside this plugin (extracted to IntelliJ's
...
class CimnotebookServerConnectionProvider : LanguageServerFactory {
     ...
     *
     * Resolution order:
     *  1. Explicit value from Settings > Tools > CIMNotebook > Java executable.
    ...
    private fun resolveServerJar(configured: String): String {
        ...
        if (configured.isNotBlank()) {
            ...
            throw IllegalStateException(
                "CIMNotebook: configured server JAR not found: $configured\n" +
                    "Check Settings > Tools > CIMNotebook > Server JAR.",
            )
        }

        // 2. Extract the bundled resource to IntelliJ's system cache.
        val version = pluginVersion()
        val cacheDir = File(PathManager.getSystemPath(), "cimnotebook")
        val cached = File(cacheDir, "cimvocabcheck-lsp-$version.jar")
        if (!cached.exists()) {
            val stream =
                javaClass.classLoader.getResourceAsStream("server/cimvocabcheck-lsp.jar")
                    ?: throw IllegalStateException(
                        "CIMNotebook: cimvocabcheck-lsp.jar not found in plugin resources.\n" +
                            "Either set a path in Settings > Tools > CIMNotebook > Server JAR, " +
    ...
    private fun pluginVersion(): String =
        javaClass.classLoader
            .getResourceAsStream("cimnotebook-plugin.properties")
            ?.use { stream ->
