...
@State(
    name = "CimnotebookSettings",
    storages = [Storage("cimnotebook.xml")],
)
class CimnotebookSettings : PersistentStateComponent<CimnotebookSettings.State> {
    data class State(
        @JvmField var serverJar: String = "",
        // Empty by default so CimnotebookServerConnectionProvider.resolveJavaExecutable falls through
    ...
    /** Absolute path to cimvocabcheck-lsp.jar, or empty to use the bundled JAR. */
    ...
    companion object {
        @JvmStatic
        fun getInstance(): CimnotebookSettings = service()
    }
}
