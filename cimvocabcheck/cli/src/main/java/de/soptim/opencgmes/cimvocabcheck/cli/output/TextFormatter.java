 ...
 *
 * <pre>
 * query.rq:3:12: error: [UNKNOWN_CLASS] Class &lt;...&gt; does not exist in profile [Equipment].
  ...
  private void writeSummary(List<FileResult> results) {
    ...
    if (total == 1) {
      ...
      if (errors == 0 && warnings == 0) {
        ...
        out.printf("%s: %d error(s), %d warning(s)%n", results.get(0).source(), errors, warnings);
      }
    } else {
      // Multi-file summary.
      out.printf("──────────────────────────────────────────%n");
      out.printf(
          "%d file(s): %d valid, %d invalid — %d error(s), %d warning(s)%n",
