 ...
 * locator tries — in order — the three forms a SPARQL query can use to refer to a term:
 *
 * <ol>
 *   <li>{@code <full-IRI>} — the most precise form, tried first.
 *   <li>{@code prefix:local} — using every prefix declaration in the query's {@link PrefixMapping}
 *       whose namespace IRI is a prefix of the target IRI.
 *   <li>The SPARQL shorthand {@code a} — only when the target is {@code rdf:type}.
 * </ol>
 *
 * <p>The earliest match across all three forms wins. If the term appears multiple times, {@link
 * #locate} returns the first occurrence; {@link #locateWithHint} picks the occurrence that appears
 * closest (by character offset) to a given <em>hint</em> node — typically the subject or object
 * from the same triple pattern.
 *
 * <p>The {@code a} keyword detection is regex-based and intentionally conservative: it only matches
 * {@code a} that is preceded by start-of-text / whitespace / {@code .} / {@code ;} and followed by
 * a predicate-position token. False positives inside string literals are possible but rare in
 * practice — the locator is best-effort.
 */
public final class SourceLocator {
  ...
  /** A 1-based line/column position; {@code null} fields denote an unknown coordinate. */
   ...
   * @param prefixes prefix map from the parsed {@link org.apache.jena.query.Query}, may be {@code
   *     null}
   */
  public static Location locate(String query, Node term, PrefixMapping prefixes) {
    if (query == null || term == null || !term.isURI()) {
      return UNKNOWN;
    }
  ...
  /**
   * Like {@link #locate} but uses {@code hint} (typically the subject node from the same triple
   * pattern) to prefer the occurrence of {@code term} that appears closest in the source text to
   * that hint.
   *
   * <p>The hint may be a SPARQL variable (looked up as {@code ?name} / {@code $name}), a URI node
   * (looked up as full IRI or prefixed form), or a blank node (not searchable — falls back to first
   * occurrence). When the hint is {@code null}, or no occurrences of the hint can be found in the
   * text, the method falls back to the first occurrence of {@code term}.
   *
   * @param query original query text, may be {@code null}
  ...
  public static Location locateWithHint(
    ...
    if (termOffsets.size() == 1 || hint == null) {
      return toLineColumn(query, termOffsets.get(0));
    }
    ...
    for (int hintOffset : hintOffsets) {
      int hintLine = lineOf(query, hintOffset);
      for (int termOffset : termOffsets) {
        if (lineOf(query, termOffset) == hintLine) {
          return toLineColumn(query, termOffset);
        }
      }
    }
  ...
  private static List<Integer> findAllTermOffsets(String query, Node term, PrefixMapping prefixes) {
    ...
    collectIriOffsets(query, iri, prefixes, offsets);
    if (RDF_TYPE_URI.equals(iri)) {
      collectAllRdfTypeKeyword(query, offsets);
    }
  ...
  /**
   * Collects text offsets for {@code hint}: variables as {@code ?name}/{@code $name}, URI nodes as
   * full-IRI or prefixed form. Blank nodes and literals are not searchable.
   */
  private static List<Integer> findAllNodeOffsets(String query, Node hint, PrefixMapping prefixes) {
    ...
    if (hint.isVariable()) {
      ...
      collectIriOffsets(query, hint.getURI(), prefixes, offsets);
    }
  ...
  /**
   * Appends offsets of {@code iri} to {@code out}, both as a full {@code <iri>} occurrence and as
   * each {@code prefix:local} form derivable from {@code prefixes}.
   */
  private static void collectIriOffsets(
      String query, String iri, PrefixMapping prefixes, List<Integer> out) {
    collectAllFullIri(query, iri, out);
    if (prefixes == null) {
      return;
    }
    for (var e : prefixes.getNsPrefixMap().entrySet()) {
      String ns = e.getValue();
      if (ns == null || ns.isEmpty() || !iri.startsWith(ns)) {
        continue;
      }
      ...
      collectAllWholeToken(query, e.getKey() + ":" + local, out);
    }
  }
  ...
  private static void collectAllWholeToken(String text, String token, List<Integer> out) {
    ...
    while ((idx = text.indexOf(token, from)) >= 0) {
      ...
      if (okBefore && okAfter && !isInComment(text, idx)) {
        out.add(idx);
      }
      from = idx + 1;
    }
  ...
  private static void collectAllRdfTypeKeyword(String query, List<Integer> out) {
    Matcher m = A_KEYWORD.matcher(query);
    while (m.find()) {
      int start = m.start();
      if (isInComment(query, start)) {
        continue;
      }
      if (start == 0) {
        out.add(start);
   ...
   * double-quoted, and {@code <...>} IRI-bracket states so that a {@code #} inside a string literal
   * or an IRI (e.g. {@code <http://example.org/ns#term>}) does not falsely trigger.
   */
