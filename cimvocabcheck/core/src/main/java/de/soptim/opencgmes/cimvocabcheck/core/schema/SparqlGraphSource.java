 ...
 * in-memory Jena dataset. It exposes only the four operations CIMVocabCheck needs to load a schema
 * from an endpoint and auto-detect which instance graph belongs to which profile:
 *
 * <ul>
 *   <li>{@link #listSchemaGraphs()} — named graphs that declare a class or ontology (the schema);
 *   <li>{@link #listNonEmptyGraphs()} — every named graph that holds at least one triple;
 *   <li>{@link #fetchGraph(String)} — download one named graph in full (used for schema graphs);
 *   <li>{@link #sampleTerms(String, int)} — a bounded sample of the predicates and {@code rdf:type}
 *       objects in a graph, used to classify it without downloading its instance data.
 * </ul>
 *
 * <p>Implementations are single-threaded per instance and should be used inside a
 * try-with-resources block so any underlying HTTP resources are released.
 */
public interface SparqlGraphSource extends AutoCloseable {
   ...
   * every {@code rdf:type} object. Returned terms are URI nodes only (blank nodes and literals are
   * skipped). At most {@code limit} terms are returned — enough to classify the graph by the schema
   * profile its terms belong to, without scanning the whole graph.
   */
