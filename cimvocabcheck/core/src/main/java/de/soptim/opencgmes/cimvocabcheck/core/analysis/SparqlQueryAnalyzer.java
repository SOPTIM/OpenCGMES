...
public final class SparqlQueryAnalyzer {
   ...
   * {@code https://opencgmes.soptim.de/cimvocabcheck/base/EQ}, which callers can match by
   * prepending this prefix to a relative config key.
   *
   ...
   * {@code <base/uuid:x>}. An {@code https://} base cannot collide this way: {@code urn:}
   * references keep their scheme, and absolute {@code http(s)://} references carry an authority and
   * so are left untouched, while only genuinely relative references are resolved against it.
   */
