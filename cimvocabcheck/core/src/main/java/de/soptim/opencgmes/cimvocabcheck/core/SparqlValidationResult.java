 ...
 * with the shape documented in the module README.
 *
 * @param query the original query string as supplied by the caller
 * @param queryPlan formatted SPARQL algebra plan in Jena SSE syntax, or {@code null} when the query
 *     could not be parsed at all
 * @param annotations ordered list of findings; empty list means "no problems"
 */
public record SparqlValidationResult(
    String query, String queryPlan, List<SparqlValidationAnnotation> annotations) {

  /** Canonical constructor; requires a query and defensively copies the annotation list. */
  public SparqlValidationResult {
    Objects.requireNonNull(query, "query");
    annotations = annotations == null ? List.of() : List.copyOf(annotations);
  }

  /** Returns whether no annotation has {@link SparqlValidationSeverity#ERROR}. */
  public boolean isValid() {
    return annotations.stream().noneMatch(a -> a.severity() == SparqlValidationSeverity.ERROR);
  }

  /**
   * Returns {@code true} iff the annotation list, after applying {@code level}, contains no {@link
   * SparqlValidationSeverity#ERROR} entries.
   *
   * <p>Equivalent to {@code level.apply(annotations()).stream().noneMatch(ERROR)}.
   */
