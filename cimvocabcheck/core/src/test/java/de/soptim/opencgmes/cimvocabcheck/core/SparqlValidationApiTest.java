...
/**
 * Acceptance tests for {@link SparqlValidationApi}: existence checks, semantic checks, scope
 * overloads, dependency extraction.
 */
public class SparqlValidationApiTest {
  ...
  public void setUp() {
    ...
    RdfsSchemaIndex index =
        RdfsSchemaIndex.builder()
            ...
            .addProfile(PROFILE_TP, List.of(CLASS_VOLTAGE_LEVEL), List.of(PROP_NOMINAL_V))
            .build();
  ...
  public void validQueryAgainstAllProfiles() {
    var r =
        api.validateSparql(
            PREAMBLE + "SELECT * WHERE { ?s a cim:ACLineSegment ; cim:ACLineSegment.r ?r . }");
    assertNoErrors(r);
    assertNotNull("query plan should be present", r.queryPlan());
    assertTrue(
        "plan should mention BGP", r.queryPlan().contains("bgp") || r.queryPlan().contains("BGP"));
  }
  ...
  public void propertyExistsInOtherProfile() {
    ...
    var r =
        api.validateSparql(
            PREAMBLE + "SELECT * WHERE { ?x cim:VoltageLevel.nominalVoltage ?v . }",
            List.of(VersionIri.of(PROFILE_EQ)));
    SparqlValidationAnnotation a = expectSingle(r, SparqlValidationCode.UNKNOWN_PROPERTY);
    assertEquals(1, a.foundInOtherProfiles().size());
    assertEquals(PROFILE_TP, a.foundInOtherProfiles().iterator().next().iri());
    assertTrue("message should hint at TP", a.message().contains("Topology/1.0"));
  }
  ...
  public void classExistsInOtherProfile() {
    var r =
        api.validateSparql(
            PREAMBLE + "SELECT * WHERE { ?x a cim:VoltageLevel . }",
            List.of(VersionIri.of(PROFILE_EQ)));
    SparqlValidationAnnotation a = expectSingle(r, SparqlValidationCode.UNKNOWN_CLASS);
    assertEquals(PROFILE_TP, a.foundInOtherProfiles().iterator().next().iri());
  }
  ...
  public void namedGraphScopeUsesMappedProfiles() {
    ...
    Map<Node, Collection<VersionIri>> map = Map.of(graphA, List.of(VersionIri.of(PROFILE_EQ)));
    var r =
        api.validateSparql(
            PREAMBLE + "SELECT * WHERE { GRAPH <urn:graph:a> { ?x a cim:ACLineSegment . } }", map);
    assertNoErrors(r);

    // Same query, but the class only lives in TP → should now fail.
    Map<Node, Collection<VersionIri>> mapB = Map.of(graphA, List.of(VersionIri.of(PROFILE_EQ)));
    var r2 =
        api.validateSparql(
            PREAMBLE + "SELECT * WHERE { GRAPH <urn:graph:a> { ?x a cim:VoltageLevel . } }", mapB);
  ...
  public void graphUsedButNotConfigured() {
    ...
    Map<Node, Collection<VersionIri>> map = Map.of(knownGraph, List.of(VersionIri.of(PROFILE_EQ)));
    var r =
        api.validateSparql(
            PREAMBLE + "SELECT * WHERE { GRAPH <urn:graph:unknown> { ?x a cim:ACLineSegment . } }",
            map);
    boolean found =
  ...
  public void variableGraphWithUnknownClassProducesAnnotationNotCrash() {
    ...
    var r =
        api.validateSparql(
            PREAMBLE + "SELECT * WHERE { GRAPH ?g { ?x a cim:DoesNotExist . } }", map);
    SparqlValidationAnnotation a = expectSingle(r, SparqlValidationCode.UNKNOWN_CLASS);
    assertEquals(CIM + "DoesNotExist", a.term().getURI());
  }
  ...
  public void variableGraphDoesNotProduceFalseErrors() {
    ...
    var r =
        api.validateSparql(
            PREAMBLE
                + "SELECT * WHERE { GRAPH ?g { ?x a cim:ACLineSegment ; cim:ACLineSegment.r ?r . }"
                + " }",
            map);
    assertNoErrors(r);
  }
  ...
  public void variableClassProducesWarning() {
    ...
    boolean warn =
        r.annotations().stream()
            .anyMatch(
                a ->
                    a.code() == SparqlValidationCode.UNSUPPORTED_DYNAMIC_PROPERTY
  ...
  public void subqueryIsAnalyzed() {
    var r =
        api.validateSparql(
            PREAMBLE + "SELECT * WHERE { { SELECT ?s WHERE { ?s a cim:DoesNotExist . } } }");
    SparqlValidationAnnotation a = expectSingle(r, SparqlValidationCode.UNKNOWN_CLASS);
    assertEquals(CIM + "DoesNotExist", a.term().getURI());
  }
  ...
  public void propertyPathSegmentIsValidated() {
    var r =
        api.validateSparql(PREAMBLE + "SELECT * WHERE { ?s cim:ACLineSegment.r / cim:typo ?o . }");
    SparqlValidationAnnotation a = expectSingle(r, SparqlValidationCode.UNKNOWN_PROPERTY);
  ...
  public void scopedPropertyDepsFilterByProfileList() throws InvalidQueryException {
    ...
    String q =
        PREAMBLE
            + "SELECT * WHERE { ?s cim:ACLineSegment.r ?r ; cim:VoltageLevel.nominalVoltage ?v . }";
    ...
    assertFalse(
        "TP property must be excluded", eqOnly.contains(NodeFactory.createURI(PROP_NOMINAL_V)));

    Collection<Node> both =
        api.getPropertyDependencies(
            q, List.of(VersionIri.of(PROFILE_EQ), VersionIri.of(PROFILE_TP)));
  ...
  public void scopedClassDepsFilterByProfileList() throws InvalidQueryException {
    ...
    assertFalse(
        "TP class must be excluded", eqOnly.contains(NodeFactory.createURI(CLASS_VOLTAGE_LEVEL)));
  }
  ...
  public void scopedPropertyDepsFilterByNamedGraphMap() throws InvalidQueryException {
    ...
    assertTrue(
        "PROP_R inside EQ graph must be included", deps.contains(NodeFactory.createURI(PROP_R)));
    // PROP_NOMINAL_V in default-graph context → scope falls back to union of map profiles (EQ only)
    // → excluded
    assertFalse(
        "PROP_NOMINAL_V not in EQ profile must be excluded",
  ...
  public void scopedClassDepsFilterByNamedGraphMap() throws InvalidQueryException {
    ...
    assertTrue(
        "EQ class inside mapped graph must be included",
        deps.contains(NodeFactory.createURI(CLASS_AC_LINE)));
    assertFalse(
        "TP class not in EQ profile must be excluded",
  ...
  public void scopedProfileDepsFilterByProfileList() throws InvalidQueryException {
    ...
    Collection<VersionIri> eqOnly =
        api.getProfileDependencies(q, List.of(VersionIri.of(PROFILE_EQ)));
    assertTrue(eqOnly.contains(VersionIri.of(PROFILE_EQ)));
    assertFalse(
        "TP profile out of scope must be excluded", eqOnly.contains(VersionIri.of(PROFILE_TP)));
  }
  ...
  public void scopedProfileDepsFilterByNamedGraphMap() throws InvalidQueryException {
    Node graphEq = NodeFactory.createURI("urn:graph:eq");
    Map<Node, Collection<VersionIri>> map = Map.of(graphEq, List.of(VersionIri.of(PROFILE_EQ)));
    String q = PREAMBLE + "SELECT * WHERE { GRAPH <urn:graph:eq> { ?s a cim:ACLineSegment } }";
  ...
  public void graphDepsWithProfileListReturnsAllGraphReferences() throws InvalidQueryException {
    ...
    String q =
        PREAMBLE
            + "SELECT * WHERE { GRAPH <urn:graph:a> { ?s ?p ?o } GRAPH <urn:graph:b> { ?x ?y ?z }"
            + " }";
    Collection<Node> withProfiles = api.getGraphDependencies(q, List.of(VersionIri.of(PROFILE_EQ)));
    ...
    assertEquals("profile list must not narrow syntactic graph references", noArg, withProfiles);
  }
  ...
  public void constructTemplateUnknownClassEmitsError() {
    var r =
        api.validateSparql(
            PREAMBLE + "CONSTRUCT { ?s a cim:DoesNotExist } WHERE { ?s a cim:ACLineSegment }");
    SparqlValidationAnnotation a = expectSingle(r, SparqlValidationCode.UNKNOWN_CLASS);
    assertEquals(CIM + "DoesNotExist", a.term().getURI());
    assertFalse(r.isValid());
  }
  ...
  public void constructTemplateUnknownPropertyEmitsError() {
    var r =
        api.validateSparql(
            PREAMBLE + "CONSTRUCT { ?s cim:doesNotExist ?o } WHERE { ?s cim:ACLineSegment.r ?o }");
    SparqlValidationAnnotation a = expectSingle(r, SparqlValidationCode.UNKNOWN_PROPERTY);
    assertEquals(CIM + "doesNotExist", a.term().getURI());
  }
  ...
  public void constructTemplateDependenciesAreTracked() throws InvalidQueryException {
    Collection<Node> classes =
        api.getClassDependencies(
            PREAMBLE + "CONSTRUCT { ?s a cim:ACLineSegment } WHERE { ?s a cim:VoltageLevel }");
  ...
  public void existsInBindProducesUnknownPropertyError() {
    var r =
        api.validateSparql(
            PREAMBLE + "SELECT * WHERE { BIND(EXISTS { ?s cim:doesNotExist ?o } AS ?has) }");
    SparqlValidationAnnotation a = expectSingle(r, SparqlValidationCode.UNKNOWN_PROPERTY);
    assertEquals(CIM + "doesNotExist", a.term().getURI());
  }
  ...
  public void existsInBindPropertyDependencyTracked() throws InvalidQueryException {
    Collection<Node> props =
        api.getPropertyDependencies(
            PREAMBLE + "SELECT * WHERE { BIND(EXISTS { ?s cim:ACLineSegment.r ?o } AS ?has) }");
    assertTrue(
        "EXISTS in BIND must contribute to property dependencies",
