...
public class ShaclValidationTest {
  ...
  public void setUp() {
    RdfsSchemaIndex index =
        RdfsSchemaIndex.builder()
            .addProfile(
                PROFILE_EQ, List.of(CLASS_AC_LINE), List.of(PROP_NAME, PROP_R, PROP_EQ_CONTAINER))
  ...
  public void extractorFindsAllFourEmbeddings() {
    ...
    assertEquals(
        "should find 1 ASK, 1 CONSTRUCT and 2 SELECT queries (target + constraint)",
        4,
        queries.size());
    ...
    long constructs =
        queries.stream().filter(q -> q.kind() == EmbeddedSparql.Kind.CONSTRUCT).count();
  ...
  public void prefixesAreResolvedFromShPrefixes() {
    ...
    assertEquals(
        "cim should resolve to the CIM100 namespace",
        "http://iec.ch/TC57/CIM100#",
        first.prefixes().get("cim"));
    assertTrue(
        "renderedQuery must prepend the resolved PREFIX declaration",
  ...
  public void validShapesGraphProducesNoErrors() {
    ...
    assertTrue(
        "expected no ERROR annotations in any embedded query; got: " + describe(r), r.isValid());
  ...
  public void crossProfileShapeNeedsBothProfiles() {
    ...
    assertEquals(
        "shape uses cim:ACLineSegment.r (EQ) and cim:VoltageLevel.nominalVoltage (TP)",
        2,
        profiles.size());
  ...
  public void restrictedProfileDependenciesHidesOthers() {
    Graph g = loadShapes("shacl/shapes-cross-profile.ttl");
    Collection<VersionIri> restricted =
        api.getShaclProfileDependencies(g, List.of(VersionIri.of(PROFILE_EQ)));
  ...
  public void validShapesGraph_noShapeAnnotations() {
    ...
    var r = api.validateShacl(g);
    assertTrue(
        "shapes-basic.ttl contains only valid CIM terms in shape positions",
  ...
  public void alternativePath_unknownNamespaceAliasOfKnownProperty_notReported() {
    ...
    assertFalse(
        "namespace aliases of a known property in sh:alternativePath must not be reported",
        spurious);
  }
  ...
  public void totalAnnotationsCountsShapeAndEmbedded() {
    ...
    int embeddedOnly =
        r.embeddedResults().stream().mapToInt(e -> e.result().annotations().size()).sum();
    assertEquals(
        "totalAnnotations must equal shape + embedded counts", total, shapeOnly + embeddedOnly);
  }
  ...
  public void shaclClassDependenciesIncludesStructuralTerms() {
    ...
    boolean foundAcLine =
        classes.stream().anyMatch(n -> n.isURI() && n.getURI().equals(CLASS_AC_LINE));
  ...
  public void shaclPropertyDependenciesIncludesStructuralTerms() {
    ...
    Graph g = loadShapes("shacl/shapes-structural-errors.ttl");
    Collection<org.apache.jena.graph.Node> props = api.getShaclPropertyDependencies(g);
    boolean foundPropR = props.stream().anyMatch(n -> n.isURI() && n.getURI().equals(PROP_R));
  ...
  public void shaclProfileDependenciesIncludesStructuralTerms() {
    ...
    Collection<VersionIri> profiles = api.getShaclProfileDependencies(g);
    assertTrue(
        "getShaclProfileDependencies must include EQ from structural sh:path terms",
  ...
  public void pathVariable_isSubstitutedFromShPath() {
    ...
    assertFalse(
        "$PATH must be substituted with sh:path URI; no UNSUPPORTED_DYNAMIC_PROPERTY expected",
        dynamic);
  }
  ...
  public void pathVariable_shPathsFieldPopulated() {
    ...
    var queries = api.extractShaclSparql(g);
    boolean hasPropR =
        queries.stream().anyMatch(q -> q.shPaths().contains(NodeFactory.createURI(PROP_R)));
  ...
  private static Graph loadShapes(String resourcePath) {
    try (InputStream in =
        ShaclValidationTest.class.getClassLoader().getResourceAsStream(resourcePath)) {
      if (in == null) {
  ...
  private static String describe(ShaclValidationResult r) {
