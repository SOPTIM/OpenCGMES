 ...
 *
 * <p>Each test pins a specific Jena algebra op type or SPARQL construct variant and would fail if
 * the corresponding branch is removed from {@code AlgebraAnalysisVisitor} or {@code
 * collectPathUris()}.
 *
 * <p>Coverage map:
 *
 * <ul>
 *   <li>Property paths: P_ReverseLink, P_Alt, P_ZeroOrMoreN, P_OneOrMoreN, P_ZeroOrOne,
 *       P_NegPropSet, P_Mod, P_Seq
 *   <li>Query modifiers: OpDistinct, OpReduced, OpOrder, OpSlice, OpGroup
 *   <li>Query forms: ASK, DESCRIBE, CONSTRUCT WHERE
 *   <li>Inline data: OpTable (VALUES)
 *   <li>Remote endpoints: OpService
 *   <li>Negative patterns: FILTER NOT EXISTS, MINUS
 *   <li>Multi-segment SELECT (;-separated)
 * </ul>
 */
public class AlgebraTraversalTest {
  ...
  public void reverseLinkPath_unknownPropertyDetected() {
    var r = api.validateSparql(PREAMBLE + "SELECT * WHERE { ?o ^cim:doesNotExist ?s . }");
  ...
  public void alternativePath_unknownSecondBranchDetected() {
    var r =
        api.validateSparql(
            PREAMBLE + "SELECT * WHERE { ?s cim:ACLineSegment.r|cim:doesNotExist ?o . }");
    assertHasUnknownProperty(r, CIM + "doesNotExist");
  }
  ...
  public void alternativePath_unknownFirstBranchDetected() {
    var r =
        api.validateSparql(
            PREAMBLE + "SELECT * WHERE { ?s cim:doesNotExist|cim:ACLineSegment.r ?o . }");
    assertHasUnknownProperty(r, CIM + "doesNotExist");
  }
  ...
  public void zeroOrMorePath_unknownPropertyDetected() {
    var r = api.validateSparql(PREAMBLE + "SELECT * WHERE { ?s cim:doesNotExist* ?o . }");
    assertHasUnknownProperty(r, CIM + "doesNotExist");
  }
  ...
  public void fixedRangeQuantifierPath_unknownPropertyDetected() {
    var r = api.validateSparql(PREAMBLE + "SELECT * WHERE { ?s cim:doesNotExist{1,3} ?o . }");
    assertHasUnknownProperty(r, CIM + "doesNotExist");
  }
  ...
  public void validPropertyPath_noErrors() {
    var r = api.validateSparql(PREAMBLE + "SELECT * WHERE { ?s cim:ACLineSegment.r* ?o . }");
    assertNoErrors(r);
  }
  ...
  public void selectDistinct_unknownClassDetected() {
    var r = api.validateSparql(PREAMBLE + "SELECT DISTINCT * WHERE { ?s a cim:DoesNotExist . }");
  ...
  public void selectReduced_unknownClassDetected() {
    var r = api.validateSparql(PREAMBLE + "SELECT REDUCED * WHERE { ?s a cim:DoesNotExist . }");
    assertHasUnknownClass(r, CIM + "DoesNotExist");
  }
  ...
  public void orderBy_unknownClassDetected() {
    var r = api.validateSparql(PREAMBLE + "SELECT * WHERE { ?s a cim:DoesNotExist . } ORDER BY ?s");
    assertHasUnknownClass(r, CIM + "DoesNotExist");
  }
  ...
  public void limit_unknownClassDetected() {
    var r = api.validateSparql(PREAMBLE + "SELECT * WHERE { ?s a cim:DoesNotExist . } LIMIT 10");
    assertHasUnknownClass(r, CIM + "DoesNotExist");
  }
  ...
  public void groupBy_unknownClassInWhereDetected() {
    var r =
        api.validateSparql(
            PREAMBLE + "SELECT ?s (COUNT(*) AS ?c) WHERE { ?s a cim:DoesNotExist . } GROUP BY ?s");
    assertHasUnknownClass(r, CIM + "DoesNotExist");
  }
  ...
  public void orderByAndLimit_unknownPropertyDetected() {
    var r =
        api.validateSparql(
            PREAMBLE + "SELECT * WHERE { ?s cim:doesNotExist ?o . } ORDER BY ?s LIMIT 5");
    assertHasUnknownProperty(r, CIM + "doesNotExist");
  }
  ...
  public void askQuery_unknownClassDetected() {
    var r = api.validateSparql(PREAMBLE + "ASK WHERE { ?s a cim:DoesNotExist . }");
    assertHasUnknownClass(r, CIM + "DoesNotExist");
  }
  ...
  public void askQuery_validTerms_noErrors() {
    var r =
        api.validateSparql(
            PREAMBLE + "ASK WHERE { ?s a cim:ACLineSegment ; cim:ACLineSegment.r ?r . }");
    assertNoErrors(r);
  }
  ...
  public void describeQuery_unknownPropertyInWhereDetected() {
    var r = api.validateSparql(PREAMBLE + "DESCRIBE ?s WHERE { ?s cim:doesNotExist ?o . }");
    assertHasUnknownProperty(r, CIM + "doesNotExist");
  }

  /**
   * CONSTRUCT WHERE (compact form) — the WHERE pattern doubles as the template; both the WHERE Op
   * tree and the template quads walk must catch the unknown class.
   */
  @Test
  public void constructWhere_unknownClassDetected() {
    var r = api.validateSparql(PREAMBLE + "CONSTRUCT WHERE { ?s a cim:DoesNotExist . }");
    assertHasUnknownClass(r, CIM + "DoesNotExist");
  }
  ...
  public void constructWhere_validTerms_noErrors() {
    var r =
        api.validateSparql(
            PREAMBLE + "CONSTRUCT WHERE { ?s a cim:ACLineSegment ; cim:ACLineSegment.r ?r . }");
    assertNoErrors(r);
  }
  ...
  public void valuesAlone_noFalsePositive() {
    var r = api.validateSparql(PREAMBLE + "SELECT * WHERE { VALUES ?x { <urn:ex> } }");
    assertNoErrors(r);
  }
  ...
  public void valuesFollowedByBgp_unknownClassDetected() {
    var r =
        api.validateSparql(
            PREAMBLE + "SELECT * WHERE { VALUES ?x { \"a\" } . ?s a cim:DoesNotExist . }");
    assertHasUnknownClass(r, CIM + "DoesNotExist");
  }
  ...
  public void serviceBlock_unknownClassInsideIsIgnored() {
    var r =
        api.validateSparql(
            PREAMBLE + "SELECT * WHERE { SERVICE <urn:endpoint> { ?s a cim:DoesNotExist . } }");
    long errors =
  ...
  private static void assertHasUnknownClass(SparqlValidationResult r, String classUri) {
    boolean found =
        r.annotations().stream()
            .anyMatch(
                a ->
                    a.code() == SparqlValidationCode.UNKNOWN_CLASS
                        && a.term() != null
  ...
  private static void assertHasUnknownProperty(SparqlValidationResult r, String propUri) {
    boolean found =
        r.annotations().stream()
            .anyMatch(
                a ->
                    a.code() == SparqlValidationCode.UNKNOWN_PROPERTY
                        && a.term() != null
