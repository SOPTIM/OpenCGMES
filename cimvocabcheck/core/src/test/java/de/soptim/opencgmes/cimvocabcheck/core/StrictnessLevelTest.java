...
/**
 * Unit tests for {@link StrictnessLevel}: parse, apply (filter/promote), and {@link
...
public class StrictnessLevelTest {
  ...
  public void strict_codePreservedAfterPromotion() {
    var warn =
        annotation(
            SparqlValidationSeverity.WARN, SparqlValidationCode.NODE_KIND_INCOMPATIBLE_WITH_RANGE);
  ...
  public void pedantic_promotesWarnAndInfoToError() {
    ...
    assertTrue(
        "all should be ERROR",
        out.stream().allMatch(a -> a.severity() == SparqlValidationSeverity.ERROR));
  }
  ...
  public void permissive_keepsStructuralCodesOnly() {
    var ann =
        List.of(
            ...
            annotation(SparqlValidationSeverity.WARN, SparqlValidationCode.GRAPH_NOT_CONFIGURED),
            annotation(
                SparqlValidationSeverity.WARN, SparqlValidationCode.UNSUPPORTED_DYNAMIC_PROPERTY),
            annotation(SparqlValidationSeverity.INFO, SparqlValidationCode.QUERY_IMPLIED_TYPE));
  ...
  public void result_isValid_defaultEquivalentToNoArg() {
    ...
    assertTrue(
        "WARN alone → valid under DEFAULT (overload)", result.isValid(StrictnessLevel.DEFAULT));
  }
  ...
  public void result_isValid_permissiveIgnoresSemanticErrors() {
    ...
    assertTrue(
        "semantic ERROR filtered out by PERMISSIVE", result.isValid(StrictnessLevel.PERMISSIVE));
  }

  @Test
  public void result_isValid_permissiveStillFailsOnUnknownClass() {
    var result =
        resultWith(annotation(SparqlValidationSeverity.ERROR, SparqlValidationCode.UNKNOWN_CLASS));
    assertFalse(
        "UNKNOWN_CLASS is structural → invalid even under PERMISSIVE",
