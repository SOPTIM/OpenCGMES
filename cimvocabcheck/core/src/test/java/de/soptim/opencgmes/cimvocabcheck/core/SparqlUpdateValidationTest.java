...
/**
 * Tests for SPARQL Update validation: INSERT DATA, DELETE WHERE, INSERT/DELETE ... WHERE, CREATE
 * GRAPH, DROP GRAPH, and multi-operation requests separated by {@code ;}.
 */
public class SparqlUpdateValidationTest {
  ...
  public void insertDataWithUnknownProperty_emitsError() {
    var r = api.validateSparql(PREAMBLE + "INSERT DATA { <urn:x> cim:doesNotExist \"val\" . }");
  ...
  public void insertDataWithUnknownClass_emitsError() {
    var r = api.validateSparql(PREAMBLE + "INSERT DATA { <urn:x> a cim:NoSuchClass . }");
  ...
  public void deleteDataWithValidTriples_noErrors() {
    var r = api.validateSparql(PREAMBLE + "DELETE DATA { <urn:line1> a cim:ACLineSegment . }");
    assertNoErrors(r);
  }

  @Test
  public void deleteDataWithUnknownProperty_emitsError() {
    var r = api.validateSparql(PREAMBLE + "DELETE DATA { <urn:x> cim:phantom \"val\" . }");
  ...
  public void deleteWhereWithValidPattern_noErrors() {
    var r =
        api.validateSparql(
            PREAMBLE + "DELETE WHERE { ?s a cim:ACLineSegment ; cim:ACLineSegment.r ?r . }");
    assertNoErrors(r);
  }

  @Test
  public void deleteWhereWithUnknownProperty_emitsError() {
    var r = api.validateSparql(PREAMBLE + "DELETE WHERE { ?s cim:doesNotExist ?o . }");
    expectSingle(r, SparqlValidationCode.UNKNOWN_PROPERTY);
  }
  ...
  public void createGraphReferenceIsTracked() throws InvalidQueryException {
    Collection<Node> graphs = api.getUpdateGraphDependencies("CREATE GRAPH <http://example.com/g>");
  ...
  public void dropGraphReferenceIsTracked() throws InvalidQueryException {
    Collection<Node> graphs = api.getUpdateGraphDependencies("DROP GRAPH <http://example.com/g>");
    assertTrue(graphs.contains(NodeFactory.createURI("http://example.com/g")));
  }
  ...
  public void insertIntoNamedGraph_graphIsTracked() throws InvalidQueryException {
    Collection<Node> graphs =
        api.getUpdateGraphDependencies(
            PREAMBLE + "INSERT DATA { GRAPH <http://ex.com/g> { <urn:a> a cim:ACLineSegment } }");
  ...
  public void getUpdatePropertyDependencies() throws InvalidQueryException {
    Collection<Node> props =
        api.getUpdatePropertyDependencies(
            PREAMBLE + "INSERT DATA { <urn:a> cim:ACLineSegment.r \"0.01\"^^xsd:float . }");
  ...
  public void getUpdateClassDependencies() throws InvalidQueryException {
    Collection<Node> classes =
        api.getUpdateClassDependencies(PREAMBLE + "INSERT DATA { <urn:a> a cim:ACLineSegment . }");
  ...
  public void getUpdateProfileDependencies() throws InvalidQueryException {
    Collection<VersionIri> profiles =
        api.getUpdateProfileDependencies(
            PREAMBLE
                + "INSERT DATA { <urn:a> a cim:ACLineSegment ; cim:ACLineSegment.r"
                + " \"0.1\"^^xsd:float }");
  ...
  public void getUpdateProfileDependencies_scopedToNamedGraphMap() throws InvalidQueryException {
    ...
    String u = PREAMBLE + "INSERT DATA { GRAPH <urn:graph:eq> { <urn:a> a cim:ACLineSegment } }";
  ...
  public void analyzeUpdate_returnsCorrectGraphSources() throws InvalidQueryException {
    var validator =
        new SparqlQueryValidator(
            RdfsSchemaIndex.builder()
                .addProfile(PROFILE_EQ, List.of(CLASS_AC_LINE), List.of(PROP_R))
                .build());
    SparqlUpdateAnalysis a =
        validator.analyzeUpdate(
            PREAMBLE
                + "INSERT DATA { GRAPH <http://ex.com/g> { <urn:a> a cim:ACLineSegment } } ;\n"
                + "CREATE GRAPH <http://ex.com/h>");

    long templateGraphs =
        a.graphs().stream().filter(g -> g.source() == GraphReference.Source.GRAPH_BLOCK).count();
  ...
  public void withGraphScopeAppliedToWhereAndTemplates() {
    ...
    var r =
        twoApi.validateSparql(
            PREAMBLE
                + "WITH <urn:graph:eq>\n"
                + "DELETE { ?s a cim:ACLineSegment }\n"
                + "INSERT { ?s a cim:VoltageLevel }\n"
                + "WHERE  { ?s a cim:ACLineSegment }",
            scope);
    boolean foundUnknownClass =
        r.annotations().stream()
            .anyMatch(
                a ->
    ...
    assertTrue(
        "VoltageLevel must be unknown in WITH <eq> scope, got: " + r.annotations(),
        foundUnknownClass);
  }
    ...
    var r =
        twoApi.validateSparql(
            PREAMBLE
                ...
                + "WHERE  { BIND(<urn:graph:tp> AS ?g) . ?s a cim:ACLineSegment }",
            scope);
    long unknownClassErrors =
        r.annotations().stream()
            .filter(a -> a.code() == SparqlValidationCode.UNKNOWN_CLASS)
            .count();
    assertEquals(
        "GRAPH ?g template must not be attributed to WITH scope; got: " + r.annotations(),
        0,
        unknownClassErrors);
  }
  ...
  public void clearGraphReferenceIsTracked() throws InvalidQueryException {
    Collection<Node> graphs = api.getUpdateGraphDependencies("CLEAR GRAPH <http://example.com/g>");
    assertTrue(graphs.contains(NodeFactory.createURI("http://example.com/g")));
  }
