...
public class EndpointSchemaLoaderTest {
  ...
  private static Dataset datasetWithSchema() {
    ...
    ds.addNamedModel(
        "http://ex.org/data/tp",
        parse("<http://ex.org/n1> a <http://iec.ch/TC57/CIM100#TopologicalNode> ."));
    return ds;
  }
  ...
  public void autoMapsInstanceGraphsToTheirProfile() {
    ...
    assertTrue(
        "schema graph maps to all profiles",
  ...
  public void reportsGraphsWithNoMatchingProfileAsUnmatched() {
    ...
    ds.addNamedModel(
        "http://ex.org/data/junk", parse("<http://ex.org/x> a <http://example.org/Foo> ."));
