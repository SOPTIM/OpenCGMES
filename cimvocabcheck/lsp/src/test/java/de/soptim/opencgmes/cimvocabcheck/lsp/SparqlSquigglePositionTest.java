 ...
 *
 * <p>Tests cover both standalone SPARQL (.rq/.sparql files) and embedded SPARQL inside SHACL shapes
 * (.ttl files). We call the package-visible conversion helpers directly so that the test does not
 * depend on a running LSP server or language client.
 */
public class SparqlSquigglePositionTest {
  ...
  public void shacl_seleeect_squiggleOnBadToken() {
    ...
    assertEquals(
        "squiggle line — must NOT be the sh:select line", 14, d.getRange().getStart().getLine());
  ...
  private Diagnostic firstSyntaxDiagnostic(String query) {
    ...
    SparqlValidationAnnotation syntaxAnnotation =
  ...
  public void syntaxOnlyNotice_isErrorOnFirstLine() {
    String text =
        "# [endpoint=http://localhost:3030/test/shacl]\r\n" + "SELECT * WHERE { ?s ?p ?o }";
    ...
    assertEquals("cimvocabcheck", d.getSource());
